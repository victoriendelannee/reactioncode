...
public abstract class BondMatcher {
    ...
    private static final class QueryMatcher extends BondMatcher {
        ...
        @Override
        public boolean matches(IBond bond1, IBond bond2) {
        	//System.out.println(((IQueryBond) bond1).matches(bond2) + " " +bond1.getBegin().getSymbol() + bond1.getOrder() + bond1.getEnd().getSymbol()+ " " + bond1.isAromatic() +
        	//		" " +bond2.getBegin().getSymbol() + bond2.getOrder() + bond2.getEnd().getSymbol() + " " +bond2.isAromatic());
        	//QueryBond bond = (QueryBond) bond1;
        	//System.out.println("   "+bond.getExpression());
        	
        	if (bond2 instanceof IQueryBond) 
        		return match2SMARTS((QueryBond) bond1, (QueryBond) bond2); 
        	else
         ...
         * @return
         */
        private boolean match2SMARTS(QueryBond query, QueryBond target) {
