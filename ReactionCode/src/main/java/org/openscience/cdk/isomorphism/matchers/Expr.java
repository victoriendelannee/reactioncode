...
public final class Expr {
    ...
    private boolean matches(Type type, IAtom atom, int stereo) {
        switch (type) {
            ...
            case HAS_ALIPHATIC_HETERO_SUBSTITUENT:
                ...
                int q1 = 0;
                for (IBond bond : atom.bonds())
                    q1 += (matches(Type.IS_HETERO, bond.getOther(atom), stereo) && !bond.getOther(atom).isAromatic()) ? 1 : 0;
                return q1 > 0;
            //END modification
            case ALIPHATIC_HETERO_SUBSTITUENT_COUNT:
                ...
                int q2 = 0;
                for (IBond bond : atom.bonds())
                    q2 += (matches(Type.IS_HETERO, bond.getOther(atom), stereo) && !bond.getOther(atom).isAromatic()) ? 1 : 0;
                return q2 == value;
            case HAS_HETERO_SUBSTITUENT:
                ...
                int q11 = 0;
                for (IBond bond : atom.bonds())
                    q11 += matches(Type.IS_HETERO, bond.getOther(atom), stereo) ? 1 : 0;
                return q11 > 0;
            //END modification
            case HETERO_SUBSTITUENT_COUNT:
                ...
                int q22 = 0;
                for (IBond bond : atom.bonds())
                    q22 += matches(Type.IS_HETERO, bond.getOther(atom), stereo) ? 1 : 0;
                return q22 == value;
            //MODIFICATION attribute e is ignore an true is always return
            case HAS_R_PI_ELECTRON:
            	return true;
            case R_PI_ELECTRON_COUNT:
            	return true;
            //END modification
            case INSATURATION:
                int db = 0;
    ...
    public Map<Type,List<Integer>> toMap() {
    	Map<Type,List<Integer>> map = new HashMap<Type,List<Integer>>();
    	if (type.isLogical()) {
    		if (type.equals(Type.OR) || type.equals(Type.AND)) {
    			if (left.type().isLogical())
    				exprToMap(left, map);
    			else
    				addToMap(map, left.type(), left.value());
    			exprToMap(right, map);
    		}
    	}
    	else {
    		addToMap(map, type, value);
    	}
		return map;
    }
    
    private void exprToMap(Expr exp, Map<Type,List<Integer>> map) {
    	if (exp.type().isLogical()) {
    		if (exp.type().equals(Type.OR) || exp.type().equals(Type.AND)) {
    			if (exp.left().type().isLogical())
    				exprToMap(exp.left(), map);
    			else
    				addToMap(map, exp.left().type(), exp.left().value());
    			exprToMap(exp.right(), map);
        }
    		
    	}
    	else {
    		addToMap(map, exp.type(), exp.value());
    		return;
    	}
    }
    
    private void addToMap(Map<Type,List<Integer>> map, Expr.Type type, int value) {
    	if  (!map.containsKey(type)) {
    		...
    		List<Integer> values = map.get(type);
    		if (type.hasValue())
    			values.add(value);
    		map.put(type, values);
    	}
