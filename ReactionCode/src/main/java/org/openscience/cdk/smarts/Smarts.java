            ...
            while (true) {
                ...
                switch (next()) {
                            ...
                            default:  // Z=None
                                ...
                                if (isFlavor(FLAVOR_DAYLIGHT)) {
                                    ...
                                    if (num < 0) {
                                        ...
                                        // CACTVS style ranges z{0-2}
                                        if (peek() == '{'){
                                            expr.setPrimitive(ALIPHATIC_HETERO_SUBSTITUENT_COUNT, 0);
                                            if (!parseRange(expr))
                                                return false;
                                        }
                                    } 
                    ...
                    case 'e':
                        ...
                        num = nextUnsignedInt();
                        //TODO implement An atom attribute for the ring pi electron count of all ESSSR rings the atom is part of. 
                        //If there is more than one such ring, a match in any of these is sufficient. If no number modifier is supplied, 
                        //the condition requires the presence of one or more pi electrons in the ring.
                        //a temporary Solution is emplemented, the propery is detected but ignored
                        if (num < 0) {
                        	expr = new Expr(HAS_R_PI_ELECTRON);
                            // CACTVS style ranges e{0-2}
                            if (peek() == '{'){
                                expr.setPrimitive(R_PI_ELECTRON_COUNT, 0);
                                if (!parseRange(expr))
                                    return false;
                            }
                        } 
                        else if (num == 0)
                            expr = new Expr(HAS_R_PI_ELECTRON).negate();
                        else
                            expr = new Expr(R_PI_ELECTRON_COUNT, num);
                        break;
                    case 'n':
                    ...
                    case 'z':
                        ...
                        if (num < 0) {
                            ...
                            if (peek() == '{'){
                                expr.setPrimitive(HETERO_SUBSTITUENT_COUNT, 0);
                                if (!parseRange(expr))
                                    return false;
                            }
                        } 
