...
public final class SmartsPattern extends Pattern {
     ...
     * @param builder
     */
    private SmartsPattern(final String smarts, int flavor, IChemObjectBuilder builder) {
        this.query = new QueryAtomContainer(builder);
        if (!Smarts.parse(query, smarts, flavor))
            throw new IllegalArgumentException("Could not parse SMARTS: " +
                                               smarts + "\n" +
                                               Smarts.getLastErrorMesg() + "\n" +
                                               Smarts.getLastErrorLocation());
     ...
     * @return
     */
    public static SmartsPattern create(String smarts, int flavor) {
        return new SmartsPattern(smarts, flavor, null);
    }
}
