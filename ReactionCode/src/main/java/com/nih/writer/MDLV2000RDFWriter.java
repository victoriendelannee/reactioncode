...
public class MDLV2000RDFWriter extends DefaultChemObjectWriter {

    private static ILoggingTool logger
            = createLoggingTool(MDLV2000RDFWriter.class);
    private static final Logger LOG = getLogger(MDLV2000RDFWriter.class.getName());
    private BufferedWriter writer;
    private boolean writeAromaticBondTypes = false;
    ...
    public Map<Object,Object> rdFieldsProperties = null;
     ...
     */
    public MDLV2000RDFWriter(Writer out) {
        try {
     ...
     */
    public MDLV2000RDFWriter(OutputStream output) {
     ...
     * &gt; &lt;key&gt;<br>
     * &gt; value<br>
     * if RDFileReader is used, you can get the map by reaction.getProperties();
     *
     * @param map The map to be used, map of String-String pairs
     */
    public void setRdFieldsProperties(Map map) {
    	rdFieldsProperties = map;
    }

    public boolean isWriteAromaticBondTypes() {
		return writeAromaticBondTypes;
	}

	public void setWriteAromaticBondTypes(boolean writeAromaticBondTypes) {
		this.writeAromaticBondTypes = writeAromaticBondTypes;
	}
     ...
     * @throws IOException 
     */
    private void writeReactionSet(IReactionSet reactions) throws CDKException, IOException {
    	writer.write("$RDFILE 1");
    	writer.newLine();
    	writer.write("$DATM " + LocalDate.now() + " " + LocalTime.now());
        for (Iterator<IReaction> it = reactions.reactions().iterator(); it.hasNext();) {
            IReaction reaction = it.next();
            writer.newLine();
        	writer.write("$RFMT $RIREG " + reaction.getID());
        	writer.newLine();
    ...
    private void writeReaction(IReaction reaction) throws CDKException {
        ...
        try {
            ...
            for (Entry<Object,Object> e : properties.entrySet()) {
        		...
        		if (value == null) 
        			continue;
        		if (!key.contains("$DTYPE"))
        			writer.write("$DTYPE " + key);
        		else
        			writer.write(key);
        		writer.newLine();
        ...
        for (int i = 0; i < som.getAtomContainerCount(); i++) {
            ...
            for (int j = 0; j < som.getMultiplier(i); j++) {
                ...
                try {
                    ...
                    mdlwriter.setWriteAromaticBondTypes(writeAromaticBondTypes);
